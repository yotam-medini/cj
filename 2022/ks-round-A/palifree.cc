 ...
 public:
    ...
    void drop_index(u_t idx0)
    {
        if ((!qidx.empty()) && (qidx.front() == idx0))
        {
            ...
            if (qidx.empty())
            {
                new_combs.push_back(0); // something
            }
            else
            {
...
void PaliFree::solve_naive()
{
    ...
    for (u_t bits = 0; (bits < (1u << nq)) && !any_free; ++bits)
    {
        ...
        if ((dbg_flags & 0x1) && any_free) {
            cerr << "possible @ bits=" << hex << bits << '\n'; }
    }
    possible = any_free;
}
