...
u_t max_bounded_rect(const vu_t& v, u_t* pbi=0, u_t* pei=0)
{
    ...
    {
     public:
        Node(u_t i=0, u_t w=0) : idx(i), wake(w) {}
	u_t idx; // index to v
	u_t wake;
    };
    ...
    vvu_t wake_stacks(vvu_t::size_type(sz ? : 1), vu_t());
    wake_stacks[0].push_back(0);
    u_t ib = 0;
    for (u_t i = 0; i < sz; ++i)
    {
        u_t y = v[i];
        if (inc_nodes.empty() || (y > v[inc_nodes.back().idx]))
        {
            inc_nodes.push_back(i);
        }
        while (v[inc_nodes.back().idx] > y)
        {
            inc_nodes.pop_back();
        }
	u_t rect = y * (i + 1 - ib);
