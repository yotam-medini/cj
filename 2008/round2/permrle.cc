...
class PermRLE
{
 ...
 private:
    unsigned compress_size(const char *ccs) const;
...
void PermRLE::solve_naive()
{
    ...
    unsigned best = compress_size(cs_alt);
    while (permutation_next(perm))
    {
        ...
        unsigned csize = compress_size(cs_alt);
        if (best > csize)
        {
            best = csize;
        }
...
void PermRLE::solve()
{
    ...
    while (reducing)
    {
        ...
        {
            for (unsigned e = (b + 2) % k; e != b; e = (e + 1) % k)
            {
                perm = perm0;
        ...
        for (unsigned b = 0; b < k; ++b)
        {
            for (unsigned e = (b + 1) % k; e != b; e = (e + 1) % k)
            {
...
unsigned PermRLE::compress_size(const char *ccs) const
{
    char clast = ' ';
    unsigned ret = 0;
    char c;
    for (const char *pc = ccs; (c = *pc); ++pc)
    {
        if (clast != c)
        {
            ++ret;
